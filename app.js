(() => {
  ...
  function renderCodologiaCourses() {
    ...
    availableCourses.forEach((course) => {
      ...
      label.innerHTML = `<input type="checkbox" name="codologiaCourses" value="${escapeHtml(course.value)}" class="codologia-course-checkbox"><span>${escapeHtml(course.label)}</span>`;
  ...
  function renderResult(response) {
    if (!response) {
      els.resultRoot.innerHTML = `<div class="result-card"><div class="result-headline">Нет ответа</div><h2 class="result-title">Сервер не вернул результат</h2></div>`;
      return;
    }

    if (response.errorMessage) {
      els.resultRoot.innerHTML = `<div class="result-card"><div class="result-headline">Ошибка подбора</div><h2 class="result-title">${escapeHtml(response.errorMessage)}</h2><p class="result-subtitle">${escapeHtml(response.details || "")}</p><div class="result-actions footer"><a href="tel:${escapeHtml(response.phoneTel || "")}" class="link-button secondary">Позвонить</a></div></div>`;
    ...
    if (!recommendations.length) {
      cardsHtml = `<div class="result-warn">${escapeHtml(response.noRecommendationsMessage || "Подходящих новых курсов не найдено. Менеджер поможет подобрать следующий шаг.")}</div>`;
    } else {
      recommendations.forEach((course, index) => {
        ...
        cardsHtml += `
          <article class="rec-card ${theme}">
            ...
            <div class="rec-body">
              <div class="rec-block"><strong>Почему подходит:</strong><br>${why}</div>
              <div id="${extraId}" class="rec-extra hidden">
                <div class="rec-block">
                  <strong>Что изучают:</strong>
                  <ul class="rec-list">${learns.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>
                </div>
                <div class="rec-block">
                  <strong>Почему рекомендован:</strong>
                  <ul class="rec-list">${notes.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>
                </div>
              </div>
    ...
    els.resultRoot.innerHTML = `
      <div class="result-card">
        ...
        <p class="result-subtitle">Возраст ребёнка на момент начала обучения: <strong>${escapeHtml(response.age)}</strong></p>
        ...
        <div class="result-actions footer"><button type="button" class="link-button secondary" id="retryBtn">Пройти ещё раз</button></div>
      </div>`;
